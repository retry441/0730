...
struct head * new_fb_head(char *filename, int freeid, int *offset,int sockfd)//初始化head结构体并赋值，储存块的信息
{
    struct head * p_fhead = (struct head *)malloc(head_len);
    bzero(p_fhead, head_len);
    strcpy(p_fhead->filename, filename);
    p_fhead->sockfd = sockfd;
	p_fhead->id = freeid;
...
//p_last_bs为最后一块的空间，大概率不足一个标准块的空间
{	
	/*初始化fileinfo*/
